...
- **ComboBox / SpinBox** — refuse the mouse wheel. Qt's default steps
  the value of whatever the cursor is over, so scrolling a page changes
  the language you translate into and says nothing about it. The event
  is *ignored* rather than consumed, so the page underneath still
  scrolls; the open dropdown keeps Qt's behaviour, because a long list
  has to be scrollable.
- **Progress** — pill track, `accent_fill`.
- **Application icon** — the 译 glyph, white, on a rounded tile with a
  vertical blue gradient. One idea, so it survives 16 px. Generated by
  `scripts/generate_app_icon.py`, which renders each of the nine Windows
  sizes separately rather than downsampling one; see
  [ADR 0007](decisions/0007-application-icon.md).
...
- Bare `QComboBox` / `QSpinBox`. Use `widgets.ComboBox` and
  `widgets.SpinBox`, which refuse the mouse wheel.
